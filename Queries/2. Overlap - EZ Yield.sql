...
create table #main (ident int identity, Aid int, cid int, rid int, fromdate datetime, todate datetime, buyingprice money, date datetime, newfromdate datetime, newtodate datetime, markdel bit) insert into #main ( Aid , cid , rid , fromdate , todate , buyingprice , date ) select 
	...
	inner join boardbasis b on B.BoardBasisID = R.BoardBasisId where accommodationbuildingpricerangeId in (select ID from #temp3)
                                    --  and r.roomtypeoncontractId = 247016 order by roomtypeoncontractId, convert(datetime,ABPR.fromdate)
